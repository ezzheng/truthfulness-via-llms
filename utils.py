...
def collect_acts(dataset_name, model, layer, noperiod=False, center=True, scale=False, device='cpu'):
    ...
    Loads every layer_{layer}_{batch_idx}.pt file that exists (batch_idx from filenames), in sorted order.
    """
    ...
    # Parse batch index from each filename (e.g. layer_14_25.pt -> 25) and sort so order matches data
    def batch_idx_from_path(path):
        base = os.path.basename(path)
        # layer_{layer}_{idx}.pt
        prefix, _ = base.rsplit(".", 1)
        return int(prefix.split("_")[-1])
    activation_files.sort(key=batch_idx_from_path)
    # map_location loads tensors to CPU when .pt was saved on GPU (e.g. from VM)
    acts = [t.load(p, map_location=device).to(device) for p in activation_files]
...
class DataManager:
    ...
    def add_dataset(self, dataset_name, model_size, layer, label='label', split=None, seed=None, noperiod=False, center=True, scale=False, device='cpu'):
        ...
        # Align length: activations and CSV may differ if some .pt batches are missing
        n = min(acts.shape[0], len(df))
        acts = acts[:n]
        df = df.iloc[:n]
        labels = t.tensor(df[label].values.copy(), dtype=t.float32).to(device)
        ...
        if split is not None:
            ...
            t.manual_seed(seed)
            train = t.randperm(n) < int(split * n)
            val = ~train
