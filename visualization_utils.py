...
_VIZ_ROOT = os.path.dirname(os.path.abspath(__file__))
...
class TruthData:
    ...
    def from_datasets(dataset_names, model, layer, noperiod=False, center=True, scale=False, device='cpu'):
        ...
        for dataset_name in dataset_names:
            df = pd.read_csv(os.path.join(_VIZ_ROOT, 'datasets', f"{dataset_name}.csv"))
    ...
    def plot(self, dimensions, dim_offset=0, plot_datasets=None, pca_datasets=None, arrows=[], return_df=False, **kwargs):
        ...
        acts = t.stack(acts, dim=0).to(self.device)
        ...
        pc_names = [f"PC{dim_offset+dim+1}" for dim in range(dimensions)]
        for dim in range(dimensions):
            df[pc_names[dim]] = proj[:, dim].tolist()
        ...
        # plot using plotly
        if dimensions == 2:
            fig = px.scatter(df, x=pc_names[0], y=pc_names[1], 
                             hover_name='statement', 
                             color_continuous_scale='Bluered_r',
                             **kwargs)
        elif dimensions == 3:
            fig = px.scatter_3d(df, x=pc_names[0], y=pc_names[1], z=pc_names[2], 
                                hover_name='statement', 
                                color_continuous_scale='Bluered_r',
                                **kwargs)
        else:
